 ...
 */
@WebServlet({ "/ChangeServlet", "/servlet/ChangeServlet" })
public class ChangeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		String _pro_id = request.getParameter("pro_id");
		...
		if(!(pro_batch == null || "".equals(pro_batch)
			...
			Dao dao = new Dao();
			count = dao.findChange(pro_id, pro_batch, pro_name, pro_hard, pro_soft, 
					pro_time, pro_count, pro_mac_min, pro_mac_max, pro_mac_format, 
					pro_sn_min, pro_sn_max, pro_sn_format, pro_code_count, pro_remark);
			request.setAttribute("change", count);
			request.getRequestDispatcher("product_change.jsp").forward(request, response);
		}
		else {
